...
def calibCamera(frame):
    ...
    if ret == True:
        ...
        ret, rvec, tvec = cv.solvePnP(objPoints, corners2, optimalmtx, dist)
        newimg = cv.drawFrameAxes(img, mtx, dist, rvec, tvec, 54, 2)
        cv.imshow("Frame", newimg)
        cv.waitKey(1500)
        cv.imwrite("calibration_images/calibratedImg.png", newimg)
